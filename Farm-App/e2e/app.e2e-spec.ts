...
describe('Starting tests for Farm-App', function() {
  ...
  it('website title should be Farm-App', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Farm-App');
    })
  });

  it('network-name should be farm-stock-network@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('farm-stock-network@0.0.1.bna');
    });
  });

  it('navbar-brand should be Farm-App',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Farm-App');
    });
  });
